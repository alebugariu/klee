...
namespace {
cl::opt<bool> ReadablePosix("readable-posix-inputs", cl::init(false),
		cl::desc(
...
cl::opt<bool> SilentKleeAssume("silent-klee-assume", cl::init(false),
...
void SpecialFunctionHandler::handleAliasFunction(ExecutionState &state,
	...
	KLEE_DEBUG_WITH_TYPE("alias_handling",
...
void SpecialFunctionHandler::handleReportError(ExecutionState &state,
	...
	executor.terminateStateOnError(state,
			readStringAtAddress(state, arguments[2]), Executor::ReportError,
...
void SpecialFunctionHandler::handleSetForking(ExecutionState &state,
	...
	if (ConstantExpr *CE = dyn_cast<ConstantExpr>(value)) {
		...
		executor.terminateStateOnError(state,
				"klee_set_forking requires a constant arg", Executor::User);
	}
}
...
void SpecialFunctionHandler::handlePrintRange(ExecutionState &state,
	...
	if (!isa<ConstantExpr>(arguments[1])) {
		...
		bool success __attribute__ ((unused)) = executor.solver->getValue(state,
				arguments[1], value);
		assert(success && "FIXME: Unhandled solver failure");
		bool res;
...
void SpecialFunctionHandler::handleCheckMemoryAccess(ExecutionState &state,
	...
	if (!isa<ConstantExpr>(address) || !isa<ConstantExpr>(size)) {
		executor.terminateStateOnError(state,
				"check_memory_access requires constant args", Executor::User);
	} else {
		ObjectPair op;

		if (!state.addressSpace.resolveOne(cast<ConstantExpr>(address), op)) {
			executor.terminateStateOnError(state,
					"check_memory_access: memory error", Executor::Ptr, NULL,
			...
			if (!chk->isTrue()) {
				executor.terminateStateOnError(state,
						"check_memory_access: memory error", Executor::Ptr,
						NULL, executor.getAddressInfo(state, address));
			}
		}
	}
}
	...
	for (Executor::ExactResolutionList::iterator it = rl.begin(), ie = rl.end();
		...
		if (old->readOnly) {
			executor.terminateStateOnError(*s,
					"cannot make readonly object symbolic", Executor::User);
			return;
		}
